...
- **HR zones:** Use the current seven-day window. Show each configured Apple Watch BPM range with duration plus percent of measured heart-rate time for Zones 1–5 and disclose overall workout HR coverage. When zone boundaries are missing, the only secondary action opens the focused Heart Rate Zones form; other empty zone states explain missing measured HR without showing that action.
...
- **Progressive disclosure:** Apple Watch heart-rate-zone configuration belongs to its focused Progress route. Deep rebuild remains in Settings. Recovery evidence, stream-by-stream diagnostics, source preference controls, and workout-history rows do not appear on the top-level Health destination.
