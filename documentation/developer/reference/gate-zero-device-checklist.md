...
Use a paired, recently unlocked iPhone running stable iOS 26. Health access is optional; Training Max configuration, completed-cycle proposal review, and local training workflows are supported owner-data actions in this build.
...
5. Confirm the app presents Health authorization only after Connect Health, never during preparation, and local training remains usable when postponed.
...
11. Record the release envelope checks from the [release-candidate checklist](release-candidate-checklist.md): launch/resume, mutation/query responsiveness, migration/import/export duration, peak memory, storage headroom, energy/resource pause behavior, and interruption recovery. Use one conditioning run followed by ten Release-build runs at P95; HealthKit wait time is not app-controlled latency. Include the Verification Data Envelope seed and validate the privacy-safe JSON with `scripts/check-release-envelope.py --require-protocol`.
