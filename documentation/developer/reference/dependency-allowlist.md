...
The app also uses Apple system frameworks SwiftUI, Observation, OSLog, Foundation, CryptoKit, UIKit, and HealthKit. CryptoKit provides export integrity digests; UIKit is limited to presenting the system share sheet and reading battery state for the on-demand route resource-pressure gate; HealthKit is contained inside `HealthKitAdapter`. The Health connection ships only the reviewed HealthKit entitlement and keeps HealthKit values behind application-owned interfaces.
