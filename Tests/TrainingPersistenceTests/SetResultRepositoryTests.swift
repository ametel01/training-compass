...
final class SetResultRepositoryTests: XCTestCase {
    func testConfirmedResultAndAuditSurviveRestartWithoutChangingPrescription() async throws {
        ...
        let actual = try RecordedSetResult(
            id: "result",
            sessionID: "session",
            prescriptionID: "prescription",
            result: SetResult(weight: SetResultWeight(kg: 66.25), repetitions: 6),
            recordedAt: 20,
        )
    ...
    func testAuditFailureRollsBackResultAndAuditTogether() async throws {
        ...
        let actual = try RecordedSetResult(
            ...
            sessionID: "session",
            prescriptionID: "prescription",
            result: SetResult(weight: SetResultWeight(kg: 65), repetitions: 5),
            recordedAt: 20,
        )
