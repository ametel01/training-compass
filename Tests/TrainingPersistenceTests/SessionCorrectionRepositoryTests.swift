...
final class SessionCorrectionRepositoryTests: XCTestCase {
    func testReopenCorrectAndRestartRestoresCurrentProjectionAndAuditHistory() async throws {
        ...
        let original = try RecordedSetResult(
            id: "result", sessionID: "session", prescriptionID: "prescription",
            result: SetResult(weight: SetResultWeight(kg: 65), repetitions: 5), recordedAt: 20,
        )
        ...
        let corrected = try RecordedSetResult(
            id: "result", sessionID: "session", prescriptionID: "prescription",
            result: SetResult(weight: SetResultWeight(kg: 67.5), repetitions: 7), recordedAt: 30,
        )
    ...
    func testReopenPreservesExternalLinkUntilCorrectionUnlinksItAndKeepsWorkout() async throws {
        ...
        let result = try RecordedSetResult(
            id: "result", sessionID: "session", prescriptionID: "prescription",
            result: SetResult(weight: SetResultWeight(kg: 65), repetitions: 5), recordedAt: 15,
        )
        _ = try await repository.saveSetResult(
    ...
    func testCorrectionRequiresConfirmationAndRejectsStaleCurrentProjection() async throws {
        ...
        let concurrent = try RecordedSetResult(
            id: "result", sessionID: "session", prescriptionID: "prescription",
            result: SetResult(weight: SetResultWeight(kg: 66), repetitions: 6), recordedAt: 25,
        )
        _ = try await repository.saveSetResult(
