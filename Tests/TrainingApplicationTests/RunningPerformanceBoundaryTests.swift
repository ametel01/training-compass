...
final class RunningPerformanceBoundaryTests: XCTestCase {
    func testBoundaryFiltersSourceActivityPreservesFactsAndDefaultsLatestImport() async throws {
        let first = workout(
            ...
            start: 1000, imported: 10, environment: .outdoor,
        )
        let ignored = workout(
            id: "ignored", activity: "Cycling", localDate: "2026-08-15",
            start: 2000, imported: 30, environment: .indoor,
        )
        ...
        let repository = RunningRepository(
            ...
            enrichments: [
                "first": enrichment(for: first, distance: 5000),
                "latest": enrichment(for: latestImport, distance: 4000),
            ],
            routes: ["first"],
        )
    ...
    private func workout(
        ...
        HealthWorkout(
            ...
            endDate: Date(timeIntervalSince1970: start + 1800),
            duration: 1800,
            sourceName: "Watch",
            localDate: localDate,
...
private actor RunningRepository: HealthWorkoutRepository, HealthWorkoutRouteRepository {
    ...
    func upsertHealthWorkouts(_: [HealthWorkout], reconciliationContext _: String) async throws {}
    ...
    func saveHealthWorkoutRoute(_: HealthWorkoutRoute) async throws -> Bool {
        true
    }
