...
final class HealthWorkoutBoundaryTests: XCTestCase {
    ...
    func testCurrentStatusRequiresARequestedStreamAndSameDaySuccessfulCheck() {
        ...
        XCTAssertFalse(
            checked.isCurrent(
                on: today.addingTimeInterval(86400), calendar: calendar,
            ),
        )
        XCTAssertFalse(
            HealthStreamStatus(
        ...
        async throws
    {
        ...
        let newer = HealthWorkout(
            ...
            startDate: older.startDate.addingTimeInterval(3600),
            endDate: older.endDate.addingTimeInterval(3600),
            duration: 900,
            sourceName: "Phone",
...
private actor IndependentStreamsHealthClient: HealthWorkoutClient {
    ...
    func fetchHealthPage(
        ...
        default:
            HealthWorkoutPage(workouts: [], reconciliationContext: stream.rawValue)
        }
    }
}
...
private actor ProgressCollector {
    ...
    func append(_ value: HealthWorkoutImportProgress) {
        values.append(value)
    }
}
...
private actor FlakyHealthClient: HealthWorkoutClient {
    ...
    func setFailure(for stream: HealthSyncStream, enabled: Bool) {
        if enabled {
            failures.insert(stream)
        } else {
            failures.remove(stream)
        }
    }
