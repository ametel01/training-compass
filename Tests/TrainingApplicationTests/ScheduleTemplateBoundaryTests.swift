...
final class ScheduleTemplateBoundaryTests: XCTestCase {
  func testFirstUseBuildsTheDefaultScheduleFromConfiguredLifts() async throws {
    ...
    XCTAssertEqual(
      template.sessions.map { $0.primaryLiftID }, ["squat", "press", "bench", "deadlift"])
    XCTAssertEqual(
      template.sessions.map { $0.assistanceLiftID }, ["bench", "rdl", "squat", "press"])
  }
...
private actor InMemoryTrainingRepository: TrainingRepository {
  ...
  func loadScheduleTemplate() async throws -> ScheduleTemplate? { scheduleTemplate }
