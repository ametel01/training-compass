...
final class RunningPerformanceTests: XCTestCase {
    func testOnlySourceClassifiedActivityIsAcceptedAndPaceUsesFullPrecision() throws {
        ...
        let pace = try XCTUnwrap(
            RunningPace(durationSeconds: 1500, distanceMeters: 5000.25),
        )
        XCTAssertEqual(pace.secondsPerKilometer, 299.9850007499375, accuracy: 0.0000001)
        XCTAssertEqual(pace.displayValue, "5:00 min/km")
        XCTAssertNil(RunningPace(durationSeconds: 0, distanceMeters: 5000))
        XCTAssertNil(RunningPace(durationSeconds: 1500, distanceMeters: 0))
    }

    func testRunningVolumeCountsEveryRunButSumsOnlyPositiveFacts() {
        let today = TrainingDate(year: 2026, month: 8, day: 15)
        let records = [
            record("current-complete", date: today, duration: 1000, distance: 5000),
            record("current-no-distance", date: today.adding(days: -1), duration: 900, distance: nil),
            record("current-no-duration", date: today.adding(days: -2), duration: nil, distance: 4000),
            record("baseline-a", date: today.adding(days: -7), duration: 100, distance: 1000),
            record("baseline-b", date: today.adding(days: -14), duration: 200, distance: 2000),
            record("baseline-c", date: today.adding(days: -21), duration: 300, distance: 3000),
            record("baseline-d", date: today.adding(days: -28), duration: 400, distance: 4000),
        ]
        ...
        XCTAssertEqual(volume.availableDuration.currentValue, 1900)
        XCTAssertEqual(volume.availableDistance.currentValue, 9000)
        XCTAssertEqual(volume.count.comparisonMedian, 1)
        XCTAssertEqual(volume.availableDuration.comparisonMedian, 250)
        XCTAssertEqual(volume.availableDistance.comparisonMedian, 2500)
    ...
    func testLatestImportedRunIsSelectedAndEnvironmentIsNotChanged() {
        ...
        let records = [
            record("later-date", date: date, duration: 900, distance: 3000, importedAt: 10),
            record(
                "latest-import", date: date.adding(days: -1), duration: 900, distance: 3000, importedAt: 20,
            ),
        ]
    ...
    func testComparableRunsMatchEnvironmentAndInclusiveFivePercentDistance() {
        ...
        let reference = record(
            "reference", date: date, duration: 600, distance: 10000, importedAt: 50, start: 5000,
            environment: .outdoor, heartRate: heartRate,
        )
        let inclusive = record(
            "inclusive", date: date.adding(days: -1), duration: 620, distance: 9500, importedAt: 40,
            start: 4000, environment: .outdoor, heartRate: heartRate,
        )
        let wrongEnvironment = record(
            "wrong-environment", date: date.adding(days: -2), duration: 600, distance: 10000,
            importedAt: 30,
            start: 3000, environment: .treadmill, heartRate: heartRate,
        )
        let outsideTolerance = record(
            "outside-tolerance", date: date.adding(days: -3), duration: 600, distance: 9499,
            importedAt: 20, start: 2000, environment: .outdoor, heartRate: heartRate,
        )
        let performance = RunningPerformanceCalculator().calculate(
