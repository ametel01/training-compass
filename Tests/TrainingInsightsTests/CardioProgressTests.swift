...
final class CardioProgressTests: XCTestCase {
    ...
    func testDriftIsUnavailableForShortOrSparseSessions() {
        let short = cardio("short", duration: 10 * 60, distance: 1000, samples: [])
        let sparse = cardio(
            "sparse",
            duration: 20 * 60,
            distance: 2000,
    ...
    func testEfficiencyUsesDistancePerHeartbeatAndSameActivityBaseline() throws {
        let latest = cardio(
            "latest", activity: "Running", start: 3000, duration: 600, distance: 1500,
            samples: constantSamples(bpm: 150, duration: 600, base: 3000),
        )
        let priorRun = cardio(
            "prior-run", activity: "Running", start: 2000, duration: 600, distance: 1400,
            samples: constantSamples(bpm: 140, duration: 600, base: 2000),
        )
        let ride = cardio(
            "ride", activity: "Cycling", start: 1000, duration: 600, distance: 5000,
            samples: constantSamples(bpm: 140, duration: 600, base: 1000),
        )
