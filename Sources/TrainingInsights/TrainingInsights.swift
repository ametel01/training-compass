...
public struct InsightExplanation: Codable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        try self.init(
            ...
            includedRecordIDs: container.decode([String].self, forKey: .includedRecordIDs),
            excludedRecords: container.decode([E1RMExcludedRecord].self, forKey: .excludedRecords),
            formula: formula,
            dateRange: container.decode(String.self, forKey: .dateRange),
            roundingRule: container.decode(String.self, forKey: .roundingRule),
            sourceState: sourceState,
            includedDates: container.decodeIfPresent([String].self, forKey: .includedDates) ?? [],
            sourceCoverage:
            container.decodeIfPresent(String.self, forKey: .sourceCoverage) ?? sourceState,
            calculationRule:
            container.decodeIfPresent(String.self, forKey: .calculationRule) ?? formula,
            comparisonBaseline: container.decodeIfPresent(String.self, forKey: .comparisonBaseline),
            missingData: container.decodeIfPresent([String].self, forKey: .missingData) ?? [],
            exclusions:
            container.decodeIfPresent([InsightExplanationExclusion].self, forKey: .exclusions)
                ?? [],
            lastReconciliation:
            container.decodeIfPresent(String.self, forKey: .lastReconciliation),
            configuration: container.decodeIfPresent(String.self, forKey: .configuration),
        )
    }
...
public struct E1RMProgress: Codable, Equatable, Sendable {
    ...
    public var hasLongerHistory: Bool {
...
public struct E1RMProgressCalculator: Sendable {
    ...
    public func calculate(
        ...
        for source in sources {
            ...
            for result in source.results {
                ...
                let reason: E1RMExclusionReason? = if source.weekKind.isDeload {
                    .deloadWeek
        ...
        let dateRange = if let first = observations.first?.date, let last = observations.last?.date {
            "\(first.iso8601String) through \(last.iso8601String)"
        } else {
            "No included dates"
        }
    ...
    private func bestFirst(_ lhs: E1RMObservation, _ rhs: E1RMObservation) -> Bool {
        if lhs.estimatedKg != rhs.estimatedKg {
            return lhs.estimatedKg < rhs.estimatedKg
        }
        return lhs.date < rhs.date
    }
