...
public struct RunningPace: Codable, Equatable, Sendable {
    ...
    public init?(durationSeconds: Double?, distanceMeters: Double?) {
        ...
        let seconds = durationSeconds / (distanceMeters / 1000)
        guard seconds.isFinite, seconds > 0 else { return nil }
        secondsPerKilometer = seconds
    }
...
public struct RunningPerformanceComparison: Codable, Equatable, Sendable {
    ...
    public var median: RunningComparisonBaseline? {
        baseline
    }

    public var hasPrecedingComparableRun: Bool {
        precedingComparableRun != nil
    }
}
...
public struct RunningPerformance: Codable, Equatable, Sendable {
    ...
    public var selectedComparison: RunningPerformanceComparison? {
        comparison
    }

    public var runningComparison: RunningPerformanceComparison? {
        comparison
    }
...
public struct RunningVolumeCalculator: Sendable {
    ...
    public func calculate(
        ...
        let dates = Set(inHorizon.map(\.localDate.iso8601String)).sorted()
...
public struct RunningPerformanceCalculator: Sendable {
        ...
        let candidates =
            summaries
                .filter { candidate in
                    ...
                    guard referenceRecord.startDate - candidate.record.startDate <= Double(historyDays) * 86400
                    else {
                        return false
                    }
    ...
    private func metric(
        ...
        let direction: RunningComparisonDirection = switch kind {
        case .pace:
    ...
    private func displayValue(_ value: Double, kind: MetricKind) -> Int {
        switch kind {
        case .pace, .positiveDifference: Int(value.rounded())
        }
    }
    ...
    private func comparisonMissingData(
        ...
        var missing: [String] = []
        if preceding == nil {
            missing.append("No preceding Comparable Run is available")
        }
        if baseline == nil {
    ...
    private func explanation(
        ...
        if record.durationSeconds == nil {
            missing.append("HealthKit workout duration unavailable")
        }
        if record.distanceMeters == nil {
            missing.append("HealthKit distance unavailable")
        }
        if record.elevationMeters == nil {
            missing.append("Elevation unavailable")
        }
