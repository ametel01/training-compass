...
public enum PersonalRecoveryBaselineMetric: String, Codable, CaseIterable, Equatable, Sendable {
    ...
    public static var sleepDuration: Self {
        .primarySleepDuration
    }

    public static var durationConsistency: Self {
        .sleepDurationConsistency
    }

    public static var timingConsistency: Self {
        .sleepTimingConsistency
    }

    public static var hrvSDNN: Self {
...
public struct PersonalRecoveryBaseline: Codable, Equatable, Identifiable, Sendable {
    ...
    public init(
        ...
        id = "\(metric.rawValue):\(asOfDate.iso8601String)"
        self.metric = metric
        self.asOfDate = asOfDate
        self.windowStart = windowStart
        self.windowEnd = windowEnd
        self.minimumObservationDays = minimumObservationDays
        validObservationDays = observations.count
    ...
    public var classification: PersonalRecoveryBaselineComparison {
        comparison
    }
    ...
    public var exactDifferenceFromMedian: Double? {
...
public struct PersonalRecoveryBaselineCalculator: Sendable {
    ...
    public func calculate(
        ...
        let comparison: PersonalRecoveryBaselineComparison = if let compatibleCurrent, let lower, let upper {
        ...
        let includedDates = valid.map(\.date.iso8601String)
        ...
        let currentLabel = if let compatibleCurrent {
            ...
            "No current source-comparable observation"
        }
    ...
    public func calculateProjection(
        ...
        let explanation = InsightExplanation(
            ...
            includedRecordIDs: baselines.flatMap(\.explanation.includedRecordIDs),
            excludedRecords: [],
            formula:
            ...
            includedDates: baselines.flatMap(\.explanation.includedDates),
            sourceCoverage:
            ...
            comparisonBaseline: baselines.map { "\($0.metric.displayName): \($0.comparison.displayName)" }
                .joined(separator: "; "),
            missingData: baselines.flatMap(\.explanation.missingData),
            exclusions: baselines.flatMap(\.explanation.exclusions),
            lastReconciliation: baselines.compactMap(\.explanation.lastReconciliation).sorted().last,
