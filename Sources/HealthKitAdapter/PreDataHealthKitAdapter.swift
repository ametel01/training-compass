    ...
    HealthWorkoutWriteBackClient
{
            ...
            for (index, route) in routes.sorted(by: Self.routeOrder).enumerated() {
                ...
                for try await page in routeCoordinatePages(for: route) {
                    ...
                    processingDurationMilliseconds +=
                        (ProcessInfo.processInfo.systemUptime - appendStart) * 1000
                }
                ...
                processingDurationMilliseconds +=
                    (ProcessInfo.processInfo.systemUptime - finishStart) * 1000
            }
        ...
        -> HealthWorkoutEnrichment?
    {
                    ...
                    continuation.resume(
                        returning: HealthWorkoutPage(
                            ...
                            restingHeartRateSamples: stream == .restingHeartRate
                                ? recovery.compactMap {
                                    if case let .restingHeartRate(value) = $0 {
                                        return value
                                    }
                                    return nil
                                }
                            ...
                            heartRateVariabilitySamples: stream == .heartRateVariability
                                ? recovery.compactMap {
                                    if case let .heartRateVariability(value) = $0 {
                                        return value
                                    }
                                    return nil
                                }
        ...
        private static func readTypes(_ type: HealthReadType) -> [HKObjectType] {
            ...
            case .heartRate: [HKObjectType.quantityType(forIdentifier: .heartRate)].compactMap(\.self)
            case .distance:
                [
            ...
            case .activeEnergy:
                [HKObjectType.quantityType(forIdentifier: .activeEnergyBurned)].compactMap(\.self)
            case .sleep: [HKObjectType.categoryType(forIdentifier: .sleepAnalysis)].compactMap(\.self)
            case .restingHeartRate:
                [HKObjectType.quantityType(forIdentifier: .restingHeartRate)].compactMap(\.self)
            case .heartRateVariability:
                [HKObjectType.quantityType(forIdentifier: .heartRateVariabilitySDNN)].compactMap(\.self)
            }
        }
