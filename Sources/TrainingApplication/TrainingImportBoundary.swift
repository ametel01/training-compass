...
public struct NoOpTrainingImportPhaseObserver: TrainingImportPhaseObserver {
    public init() {}
    public func didReach(_: TrainingImportPhase) throws {}
}
...
public enum TrainingImportError: Error, Equatable, Sendable {
    ...
    public var privacySafeDescription: String {
        switch self {
        case .malformedArchive: "The import archive could not be decoded."
        case .invalidManifest: "The import manifest is invalid."
        case .integrityMismatch: "The import integrity check failed."
        case .unsupportedSchema: "This import schema is not supported."
        case let .incompleteArchive(detail): "The import archive is incomplete: \(detail)."
        case let .invalidRelationship(detail):
            "The import contains an invalid relationship: \(detail)."
        case let .invariantViolation(detail):
            "The import violates a training data invariant: \(detail)."
        case .confirmationRequired: "Import confirmation is required."
        case .replacementConfirmationRequired:
            "Replacing current data requires export-first confirmation."
        case .insufficientSpace: "There is not enough storage to safely restore this archive."
        case .stagingFailed: "The import could not be prepared safely."
        case .replacementFailed: "The validated replacement could not be installed."
        case .cleanupFailed: "Temporary import files could not be cleaned up."
        }
    }
}
...
public extension TrainingReplacementImportRepository {
    func requiredImportSpaceBytes(archiveBytes: Int64) async throws -> Int64 {
...
public struct TrainingImportBoundary: Sendable {
    ...
    public static func validateAuthoritativeData(
        ...
        guard Set(names) == Self.authoritativeTableNames else {
            ...
            if !missing.isEmpty {
                throw TrainingImportError.incompleteArchive("missing \(missing)")
            }
        ...
        for table in data.tables {
            ...
            for record in table.records {
                ...
                if let value = record.fields["id"] {
                    guard case let .string(id) = value, !id.isEmpty, id == record.id else {
