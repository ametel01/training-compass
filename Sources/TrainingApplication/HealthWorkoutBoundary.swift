...
public struct HealthWorkout: Codable, Equatable, Sendable, Identifiable {
    ...
    public var environment: RunningEnvironment {
        runningEnvironment
    }

    public var sourceEnvironment: RunningEnvironment {
        runningEnvironment
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        try self.init(
            healthKitUUID: container.decode(String.self, forKey: .healthKitUUID),
            activityType: container.decode(String.self, forKey: .activityType),
            ...
            sourceBundleIdentifier: container.decodeIfPresent(
                String.self, forKey: .sourceBundleIdentifier,
            ),
            sourceProductType: container.decodeIfPresent(String.self, forKey: .sourceProductType),
            sourceOSVersion: container.decodeIfPresent(String.self, forKey: .sourceOSVersion),
            deviceName: container.decodeIfPresent(String.self, forKey: .deviceName),
            deviceModel: container.decodeIfPresent(String.self, forKey: .deviceModel),
            sourceTimeZoneIdentifier: container.decodeIfPresent(
                String.self, forKey: .sourceTimeZoneIdentifier,
            ),
            localDate: container.decode(String.self, forKey: .localDate),
            timeZoneSource: container.decode(
                HealthWorkoutTimeZoneSource.self, forKey: .timeZoneSource,
            ),
            runningEnvironment: container.decodeIfPresent(
                RunningEnvironment.self, forKey: .runningEnvironment,
            ) ?? .unspecified,
            elevationMeters: container.decodeIfPresent(Double.self, forKey: .elevationMeters),
            firstImportedAt: container.decode(Date.self, forKey: .firstImportedAt),
            reconciliationContext: container.decodeIfPresent(
                String.self, forKey: .reconciliationContext,
            ),
            appAuthoredSyncIdentifier: container.decodeIfPresent(
                String.self, forKey: .appAuthoredSyncIdentifier,
            ),
            appAuthoredSyncVersion: container.decodeIfPresent(
...
public struct HealthWorkoutProvenance: Codable, Equatable, Sendable {
    ...
    public var detailLabel: String {
        var details: [String] = []
        if let sourceProductType {
            details.append(sourceProductType)
        }
        if let deviceName {
            details.append(deviceName)
        }
        if let deviceModel, deviceModel != deviceName {
            details.append(deviceModel)
        }
        if let sourceBundleIdentifier {
            details.append(sourceBundleIdentifier)
        }
        if let sourceOSVersion {
            details.append("OS \(sourceOSVersion)")
        }
        if let sourceTimeZoneIdentifier {
            details.append(sourceTimeZoneIdentifier)
        }
        if details.isEmpty {
...
public struct HealthWorkoutPage: Codable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        try self.init(
            workouts: container.decodeIfPresent([HealthWorkout].self, forKey: .workouts) ?? [],
            nextPageToken: container.decodeIfPresent(String.self, forKey: .nextPageToken),
            anchor: container.decodeIfPresent(String.self, forKey: .anchor),
            hasLimitedHistory:
            container.decodeIfPresent(Bool.self, forKey: .hasLimitedHistory) ?? false,
            reconciliationContext:
            container.decodeIfPresent(String.self, forKey: .reconciliationContext) ?? "initial",
            deletedHealthKitUUIDs:
            container.decodeIfPresent([String].self, forKey: .deletedHealthKitUUIDs) ?? [],
            streamFacts: container.decodeIfPresent([HealthSyncFact].self, forKey: .streamFacts) ?? [],
            sleepSamples:
            container.decodeIfPresent([HealthSleepSample].self, forKey: .sleepSamples) ?? [],
            restingHeartRateSamples: container.decodeIfPresent(
                [HealthRestingHeartRateSample].self, forKey: .restingHeartRateSamples,
            ) ?? [],
            heartRateVariabilitySamples: container.decodeIfPresent(
...
public struct HealthStreamStatus: Codable, Equatable, Sendable, Identifiable {
    ...
    public var id: HealthSyncStream {
        stream
    }
    ...
    public var isCurrentToday: Bool {
        isCurrent()
    }
    ...
    public var statusLabel: String {
        ...
        if lastSuccessfulCheck == nil {
            return "Not checked yet"
        }
        if mirroredContent == .empty {
            return "Checked · successful empty"
        }
        if coverage == .limitedHistory {
            return "Checked · limited history"
        }
        return "Checked"
    }
    ...
    public var historyLabel: String {
        "History: \(coverage.displayName)"
    }

    public var contentLabel: String {
        "Content: \(mirroredContent.displayName)"
    }
    ...
    public var attentionLabel: String? {
        ...
        if coverage == .limitedHistory {
            return "Health reports limited history."
        }
        return nil
    }
}
...
public struct HealthDataStatus: Codable, Equatable, Sendable {
    ...
    public var isUpdating: Bool {
        streams.contains { $0.isUpdating }
    }

    public var hasActionableAttention: Bool {
        streams.contains { $0.attentionLabel != nil }
    }

    public var requestedStreams: [HealthStreamStatus] {
        streams.filter(\.requested)
    }
...
public extension HealthWorkoutClient {
    ...
    func registerWorkoutObserver(
        onInvalidation _: @escaping @Sendable () async -> Void,
    ) async throws {}
    ...
    func fetchWorkoutEnrichment(for _: HealthWorkout) async -> HealthWorkoutEnrichment? {
        nil
    }
}
...
public extension HealthWorkoutRepository {
    ...
    func loadHealthSyncCheckpoint(for _: HealthSyncStream) async throws
    ...
    func saveHealthWorkoutEnrichment(_: HealthWorkoutEnrichment) async throws {}
    ...
    func updateHealthRebuildState(_: HealthRebuildState) async throws {
        throw HealthSyncError.unavailable
    }
    ...
    func loadHealthWorkoutLinkFacts(for _: String?) async throws
        -> [HealthWorkoutLinkFact]
    {
        throw HealthSyncError.unavailable
    }
    ...
    func upsertHealthRecoverySamples(
        _: [HealthRecoverySample], stream _: HealthSyncStream, reconciliationContext _: String,
    ) async throws {}

    func loadHealthRecoverySamples(for _: HealthSyncStream) async throws
        -> [HealthRecoverySample]
    {
        []
    }
...
public actor HealthWorkoutImportBoundary {
    ...
    public func authorizationSnapshot() -> HealthAuthorizationSnapshot {
        authorization
    }
        ...
        -> HealthSyncResult
    {
        guard authorization.state == .authorized else {
            if authorization.state == .postponed {
                return await HealthSyncResult(
                    trigger: trigger, pagesCommitted: 0, additionsOrReplacements: 0, deletions: 0,
                    checkpoint: nil, streamStatuses: healthDataStatus().streams,
                )
            }
            throw HealthWorkoutImportError.authorizationRequired
        }
    ...
    public func importWorkouts(
        ...
        let visible = await (try? repository.loadHealthWorkouts()) ?? []
        let limited =
    ...
    public func recoveryEvidence() async -> HealthRecoveryEvidenceSnapshot {
        ...
        let sleep = await (try? repository.loadHealthRecoverySamples(for: .sleep)) ?? []
        let resting = await (try? repository.loadHealthRecoverySamples(for: .restingHeartRate)) ?? []
        let variability = await
            (try? repository.loadHealthRecoverySamples(for: .heartRateVariability)) ?? []
        return HealthRecoveryEvidenceSnapshot(
            sleep: sleep.compactMap {
                guard case let .sleep(sample) = $0 else { return nil }
                return sample
            },
            restingHeartRate: resting.compactMap {
                guard case let .restingHeartRate(sample) = $0 else { return nil }
                return sample
            },
            heartRateVariability: variability.compactMap {
                guard case let .heartRateVariability(sample) = $0 else { return nil }
                return sample
            },
    ...
    public func healthWorkoutHistory() async throws -> HealthWorkoutHistorySnapshot {
        ...
        let deleted = await (try? repository.loadHealthWorkoutDeletionUUIDs()) ?? []
    ...
    public func isRecoveryGuidanceEnabled() -> Bool {
    ...
    private static func historyState(
        ...
        if !deletedHealthKitUUIDs.isEmpty {
            return .deleted
        }
        if events.isEmpty {
...
public actor HealthSyncCoordinator {
    ...
    public func statusSnapshot(
        ...
        for stream in requestedStreams {
            ...
            let lastCheck: Date? = switch (current.lastSuccessfulCheck, checkpointDate) {
