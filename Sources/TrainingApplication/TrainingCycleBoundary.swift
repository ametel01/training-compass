...
private extension TrainingCycle {
    func location(ofSession sessionID: String) -> (weekIndex: Int, sessionIndex: Int)? {
...
private extension TrainingWeek {
    func contains(_ date: TrainingDate) -> Bool {
...
public struct TrainingCycleActivationPreview: Equatable, Sendable {
    ...
    public var requiresDeloadConfirmation: Bool {
        cadenceChangesDeload
    }

    public var removesCustomizedDeload: Bool {
...
public struct TrainingCycleAbandonmentPreview: Equatable, Sendable {
    ...
    public var unperformedCount: Int {
        unperformedSessions.count
    }
}
...
public struct TrainingCycleHistoryEntry: Codable, Equatable, Sendable, Identifiable {
    ...
    public var lifecycleBadge: String {
        cycle.lifecycleState.displayName
    }

    public var includesDeloadBadge: Bool {
        cycle.weeks.contains(where: \.isDeload)
    }

    public var week1AnchorDate: TrainingDate {
        cycle.week1AnchorDate
    }
}
...
public extension TrainingCycleRepository {
    func loadDraftTrainingCycle() async throws -> TrainingCycle? {
        nil
    }
    ...
    func saveTrainingCycle(
        _: TrainingCycle,
        expectedBefore _: TrainingCycleSnapshot?,
        auditID _: String,
        occurredAt _: Int64,
        action _: TrainingCycleAuditAction,
    ) async throws -> TrainingCycleAuditEntry {
    ...
    func trainingCycleAuditHistory(for _: String) async throws
...
public struct TrainingCycleBoundary: Sendable {
    ...
    public func skipRemainingSessions(
        ...
        for session in preview.sessions {
            let cycle = try requireCycle(
                replacingSessions(
                    in: currentCycle,
                    weekIndex: weekIndex(for: weekID, in: currentBefore),
    ...
    public func finishWeek(
        weekID: String,
        confirmation: TrainingCycleLifecycleConfirmation,
        acknowledgeEarlierWeeks _: Bool = false,
        note: String? = nil,
    ) async throws -> TrainingCycleAuditEntry {
        guard confirmation == .confirmed else {
            throw TrainingCycleValidationError.confirmationRequired
    ...
    public func previewAbandonCycle(note _: String? = nil) async throws
    ...
    public func history() async throws -> [TrainingCycleHistoryEntry] {
        ...
        return try await cycles.asyncMap { cycle in
            let sessions = try await cycle.weeks.flatMap { week in
                ...
                try await TrainingCycleSessionHistory(
                    cycleID: cycle.id,
                    weekID: week.id,
                    weekKind: week.kind,
                    planned: session,
            ...
            return try await TrainingCycleHistoryEntry(
                cycle: cycle,
                audits: repository.trainingCycleAuditHistory(for: cycle.id),
                sessions: sessions,
            )
        }
    }

    public func loadHistory() async throws -> [TrainingCycleHistoryEntry] {
        try await history()
    }
    ...
    public func previewActivation(
        ...
        let chosenAnchor: TrainingDate = switch anchorChoice {
        case .retain, nil:
            draft.week1AnchorDate
        case let .replace(date):
            date
        }
    ...
    public func activate(
        ...
        try await confirmActivation(
            previewActivation(anchorChoice: anchorChoice),
    ...
    private func editableCycle(id: String) async throws -> TrainingCycle {
        ...
        let cycle: TrainingCycle? = if !id.isEmpty {
            [active, draft].compactMap(\.self).first(where: { $0.id == id })
        } else {
            active ?? draft
        }
    ...
    private func makeProgramEditedCycle(
        ...
        let configuredIDs = try await Set(liftRepository.loadLiftConfigurations().map(\.id))
        ...
        let weeks = try request.weeks.map { requestWeek in
            ...
            for requestSession in requestWeek.sessions {
                ...
                if let original {
                    ...
                    let prescriptions: [TrainingSetPrescription] = if existing.isActive {
    ...
    private func makePrescriptions(
        ...
        return try FiveThreeOnePrescription.specifications(for: kind, role: role).enumerated().map {
            ...
            try TrainingSetPrescription(
                id: uuidGenerator.makeUUID().uuidString,
                setNumber: index + 1,
                role: role,
                percentage: specification.percentage,
                repetitions: specification.repetitions,
                weightKg: snapshot.prescribedWeightKg(forPercentage: specification.percentage),
    ...
    private func makeCycle(
        ...
        let kinds: [TrainingWeekKind] = if includeDeload {
