...
public enum TrainingMaxProposalDecision: Codable, Equatable, Sendable {
    ...
    public static func manual(kg: Double) -> Self {
        .replace(kg: kg)
    }
}
...
public struct TrainingMaxProposalEvidence: Codable, Equatable, Sendable {
    ...
    public var e1RMObservations: [E1RMObservation] {
        eligibleE1RM
    }
    ...
    public var skippedOrOmittedWork: [TrainingMaxProposalExcludedWork] {
        excludedWork
    }
}
...
public struct TrainingMaxProposal: Codable, Equatable, Identifiable, Sendable {
    ...
    public var currentTrainingMax: Double {
        currentTrainingMaxKg
    }

    public var proposedTrainingMax: Double {
        proposedTrainingMaxKg
    }

    public var currentTMKg: Double {
        currentTrainingMaxKg
    }

    public var proposedTMKg: Double {
        proposedTrainingMaxKg
    }

    public var sourceCycle: String {
        sourceCycleID
    }
...
public struct TrainingMaxHistoryEntry: Codable, Equatable, Identifiable, Sendable {
    ...
    public var beforeAndAfter: (before: Double?, after: Double?) {
        (beforeKg, afterKg)
    }
}
...
public extension TrainingMaxProposalRepository {
    func loadTrainingMaxProposals(for cycleID: String) async throws -> [TrainingMaxProposal] {
        try await loadTrainingMaxProposals().filter { $0.sourceCycleID == cycleID }
    }

    func loadTrainingMaxHistory() async throws -> [TrainingMaxHistoryEntry] {
...
public extension TrainingMaxProposalRepository {
    ...
    func saveTrainingMaxProposal(
        _: TrainingMaxProposal,
        expectedBefore _: TrainingMaxProposal?,
        auditID _: String,
        occurredAt _: Int64,
        history _: TrainingMaxHistoryEntry?,
    ) async throws -> TrainingMaxProposal {
        throw TrainingMaxProposalRepositoryError.unavailable
    }

    func loadTrainingMaxHistory(for _: String?) async throws -> [TrainingMaxHistoryEntry] {
        []
    }

    func markTrainingMaxProposalsEffective(cycleID _: String) async throws {}

    func decideTrainingMaxProposal(
        _: TrainingMaxProposal,
        expectedBefore _: TrainingMaxProposal,
        configuration _: LiftConfiguration?,
        expectedConfiguration _: LiftConfigurationSnapshot?,
        auditID _: String,
        occurredAt _: Int64,
        history _: TrainingMaxHistoryEntry,
        liftAuditID _: String,
    ) async throws -> TrainingMaxProposal {
        throw TrainingMaxProposalRepositoryError.unavailable
    }
}
...
public struct TrainingMaxProposalBoundary: Sendable {
    ...
    public func proposals() async throws -> [TrainingMaxProposal] {
        ...
        return try await proposalRepository.loadTrainingMaxProposals().sorted {
            if $0.sourceCycleID != $1.sourceCycleID {
                return $0.sourceCycleID < $1.sourceCycleID
            }
            return $0.liftID < $1.liftID
        }
    }

    public func list() async throws -> [TrainingMaxProposal] {
        try await proposals()
    }
    ...
    public func previewDecision(
        ...
        case .accept:
            updated = try configuration.map {
                try LiftConfiguration(
                    ...
                    trainingMax: TrainingMax(kg: proposal.proposedTrainingMaxKg),
        ...
        case let .replace(kg):
            ...
            updated = try configuration.map {
                try LiftConfiguration(
                    id: $0.id, identity: $0.identity,
                    trainingMax: TrainingMax(kg: kg), loadingIncrement: $0.loadingIncrement,
                )
            }
        case .reject:
            updated = nil
    ...
    public func decide(
        ...
        let event: TrainingMaxHistoryEvent = switch decision {
        case .accept: .accepted
        case .reject: .rejected
        case .replace: .manuallyReplaced
        }
        ...
        -> TrainingMaxProposalEvidence
    {
        ...
        for week in cycle.weeks {
            for session in week.sessions where session.primaryLiftID == liftID {
                ...
                excluded.append(
                    contentsOf: results.filter(\.result.isFailed).map {
                        .init(
