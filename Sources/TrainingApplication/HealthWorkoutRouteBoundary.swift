...
public struct HealthWorkoutRoute: Codable, Equatable, Sendable, Identifiable {
    public static let maximumRetainedPoints = 2000
    ...
    public var points: [HealthWorkoutRoutePoint] {
        segments.flatMap(\.points)
    }

    public var originalPointCount: Int {
        segments.reduce(0) { $0 + $1.originalPointCount }
    }

    public var sources: [HealthWorkoutRouteSource] {
        segments.map(\.source)
    }
...
public struct HealthWorkoutRouteResourceSnapshot: Codable, Equatable, Sendable {
    public static let minimumAvailableStorageBytes = 500 * 1024 * 1024
...
public actor HealthWorkoutRouteBoundary {
    ...
    public func state(for healthKitUUID: String) async -> HealthWorkoutRouteSnapshot {
        if states[healthKitUUID]?.state == .loading {
            return .loading
        }
    ...
    public func openRoute(for healthKitUUID: String) async -> HealthWorkoutRouteSnapshot {
        ...
        let maximumRetainedPoints = maximumRetainedPoints
        let task = Task<HealthWorkoutRouteSnapshot, Never> {
            do {
                ...
                let postFetchMilliseconds =
                    (ProcessInfo.processInfo.systemUptime - postFetchStart) * 1000
                return .ready(
                    route,
