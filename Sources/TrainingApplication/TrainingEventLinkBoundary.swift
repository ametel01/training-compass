...
public extension TrainingEventLinkRepository {
    func createHealthWorkoutLinkFact(
        _: HealthWorkoutLinkFact,
        expectedSessionUpdatedAt _: Int64,
        expectedWorkout _: HealthWorkout,
    ) async throws -> HealthWorkoutLinkFact {
        throw TrainingEventLinkRepositoryError.unavailable
    }

    func completeSessionAndCreateHealthWorkoutLinkFact(
        completion _: CompletedSession,
        fact _: HealthWorkoutLinkFact,
        expectedSessionUpdatedAt _: Int64,
        expectedWorkout _: HealthWorkout,
    ) async throws -> TrainingEventCompletionLinkResult {
        throw TrainingEventLinkRepositoryError.unavailable
    ...
    func unlinkActiveHealthWorkoutLinkFacts(
        ...
        for link in active {
            try await unlinked.append(
                unlinkHealthWorkoutLinkFact(
...
public struct TrainingEventLinkBoundary: Sendable {
    ...
    public func timeline() async throws -> TrainingEventTimelineSnapshot {
        ...
        let persistedLinks = try await linkRepository.loadHealthWorkoutLinkFacts(for: nil)
            .filter(\.isActive)
        let writeBackRecords = await (try? writeBackBoundary?.records()) ?? []
        ...
        for cycle in cycles {
            for week in cycle.weeks {
                for session in week.sessions where session.status == .completed {
                    ...
                    let facts = try await TrainingEventSessionFacts(
                        cycleID: cycle.id,
                        weekID: week.id,
                        session: session,
                        completion: completion,
                    ...
                    if let workout {
                        consumedWorkoutIDs.insert(workout.healthKitUUID)
                    }
                    ...
                    events.append(
                        UnifiedTrainingEvent(
                            ...
                            additionalDisagreements: [link, formerLink].compactMap(\.self).compactMap {
    ...
    private static func candidate(
        ...
        if !activityMatches {
            warnings.append(.differentActivity)
        }
        if workout.localDate != session.intendedDate.iso8601String {
            warnings.append(.differentLocalDate)
        }
        if timingDifference > 6 * 60 * 60 {
            warnings.append(.distantCompletionTime)
        }
