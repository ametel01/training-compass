...
public struct SleepEpisode: Codable, Equatable, Identifiable, Sendable {
    ...
    public var midpointDate: Date {
        midpoint
    }

    public var isPrimarySleep: Bool {
        kind == .primarySleep
    }
}
...
public struct SleepEpisodeCalculator: Sendable {
    ...
    public func calculate(
        ...
        episodes.sort {
            if $0.wakeUpDate != $1.wakeUpDate {
                return $0.wakeUpDate < $1.wakeUpDate
            }
            if $0.kind != $1.kind {
                return $0.kind == .primarySleep
            }
            return $0.startDate < $1.startDate
        }

        let allDates = Set(episodes.map(\.wakeUpDate.iso8601String)).sorted()
    ...
    private struct Candidate: Sendable {
        ...
        var id: String {
            intervals.map(\.id).sorted().joined(separator: "+")
        }

        var source: HealthSleepSource {
            intervals[0].source
        }

        var startDate: Date {
            intervals.map(\.startDate).min()!
        }

        var endDate: Date {
            intervals.map(\.endDate).max()!
        }
...
public extension HealthRecoveryEvidenceSnapshot {
    var availableSleepSources: [HealthSleepSource] {
        var seen = Set<String>()
