...
private enum ApplicationAcceptanceScenario {
    ...
    init?(environmentValue: String?) {
        ...
        case "cycle-import": self = .cycleImport
        default: return nil
        }
    }
}
    ...
    RunningComparisonExclusionRepository, HealthWorkoutWriteBackRepository
{
    ...
    public init(
        ...
        applicationAcceptanceScenario = nil
    }

    private init(applicationRoot root: URL) {
        self.root = root
        bootstrapper = .init()
        phaseObserver = NoOpTrainingImportPhaseObserver()
        erasurePhaseObserver = NoOpTrainingErasurePhaseObserver()
        erasurePreferences = FoundationTrainingErasurePreferences()
        temporaryExportDirectory = FileManager.default.temporaryDirectory
            .appending(path: "TrainingCompassExports", directoryHint: .isDirectory)
        applicationAcceptanceScenario = ApplicationAcceptanceScenario(
    ...
    public func prepareStores() async throws {
        ...
        do {
            ...
            case .cycleImport:
                try await seedCycleImportAcceptanceScenario(now: Date())
            }
        } catch {
        ...
        try await stores.reconstructible.write { db in
            ...
            for sample in recoverySamples {
                let encoded = try String(
                    decoding: JSONEncoder().encode(sample), as: UTF8.self,
                )
                try db.execute(
                    sql: """
    ...
    public func upsertHealthRecoverySamples(
        ...
        try await stores.reconstructible.write { db in
            for sample in samples where sample.stream.healthSyncStream == stream {
                let encoded = try String(decoding: JSONEncoder().encode(sample), as: UTF8.self)
                try db.execute(
                    sql: """
                    INSERT INTO health_recovery_samples
                      (stream, sample_id, sample_json, sample_date, updated_at)
    ...
    public func saveHealthWorkoutEnrichment(_ enrichment: HealthWorkoutEnrichment) async throws {
        let stores = try await readyStores()
        let encoded = try String(
            decoding: JSONEncoder().encode(enrichment),
            as: UTF8.self,
        )
        try await stores.reconstructible.write { db in
            let workoutExists =
    ...
    public func saveHealthWorkoutRoute(_ route: HealthWorkoutRoute) async throws -> Bool {
        let stores = try await readyStores()
        let encoded = try String(decoding: JSONEncoder().encode(route), as: UTF8.self)
    ...
    public func beginHealthRebuild() async throws {
        let stores = try await readyStores()
        let now = Date().timeIntervalSince1970
        let encoded = try String(data: JSONEncoder().encode([HealthSyncStream]()), encoding: .utf8)!
        try await stores.reconstructible.write { db in
    ...
    public func updateHealthRebuildState(_ state: HealthRebuildState) async throws {
        let stores = try await readyStores()
        let encoded = try String(
            data: JSONEncoder().encode(state.completedStreams), encoding: .utf8,
        )!
        try await stores.reconstructible.write { db in
            try db.execute(
                sql: """
        ...
        -> [HealthWorkoutLinkFact]
    {
        let stores = try await readyStores()
        return try await stores.authoritative.read { db in
            let rows: [Row] = if let healthKitUUID {
                try Row.fetchAll(
                    db,
                    sql: """
    ...
    public func saveLiftConfiguration(
        ...
        return try await stores.authoritative.write { db in
            ...
            let historyEvent: TrainingMaxHistoryEvent = switch action {
            ...
            try db.execute(
                ...
                arguments: [
                    ...
                    Self.encodeTrainingMaxHistory(history),
                ],
            )
            ...
            if let configuration {
                ...
                try db.execute(
                    ...
                    arguments: [
                        ...
                        manualHistory.occurredAt, Self.encodeTrainingMaxHistory(manualHistory),
                    ],
                )
            }
            try db.execute(
                ...
                arguments: [
                    proposal.status.rawValue, Self.encodeTrainingMaxProposal(proposal),
                    proposal.updatedAt,
                    proposal.id,
                ],
            )
        ...
        try await stores.authoritative.write { db in
            ...
            for row in rows {
                ...
                try db.execute(
                    ...
                    arguments: [Self.encodeTrainingMaxProposal(updated), proposal.id],
                )
            }
            ...
            for row in historyRows {
                ...
                try db.execute(
                    ...
                    arguments: [Self.encodeTrainingMaxHistory(updated), history.id],
                )
            }
        }
    }
        ...
        return try await stores.authoritative.write { db in
            ...
            try db.execute(
                ...
                arguments: [
                    ...
                    action.rawValue,
                    occurredAt,
                    before.map(Self.encodeSnapshot),
                    cycleJSON,
        ...
        return try await stores.authoritative.write { db in
            ...
            try db.execute(
                ...
                arguments: [
                    ...
                    action.rawValue,
                    occurredAt,
                    current.map(Self.encodeRecordedSetResult),
                    afterJSON,
                ],
            )
        ...
        return try await stores.authoritative.write { db in
            ...
            for result in request.results {
                try db.execute(
                    ...
                    arguments: [
                        ...
                        Self.encodeRecordedSetResult(result), result.recordedAt,
                    ],
                )
            }
            ...
            try db.execute(
                ...
                arguments: [
                    ...
                    Self.encodeCorrectionSnapshot(current), Self.encodeCorrectionSnapshot(after),
                ],
            )
            try Self.reconcileHealthWorkoutWriteBack(
                db,
    ...
    private func performReplacement(
        ...
        do {
            ...
            try? staged?.close()
            try? fileManager.removeItem(at: locations.authoritativeStagingDatabase)
            if let importError = failure as? TrainingImportError {
                throw importError
            }
    ...
    private static func validateStagedStore(_ db: Database) throws {
        ...
        let liftIDs = try Set(String.fetchAll(db, sql: "SELECT id FROM lifts"))
        ...
        for row in liftRows {
            ...
            let identity: LiftIdentity? = switch kind {
            case "progression": ProgressionLift(rawValue: value).map(LiftIdentity.progression)
            case "variant": .variant(name: value)
            case "custom": .custom(name: value)
            default: nil
            }
            guard let identity,
        ...
        let templateIDs = try Set(String.fetchAll(db, sql: "SELECT id FROM schedule_templates"))
        ...
        let completedSessionIDs = try Set(
            String.fetchAll(db, sql: "SELECT session_id FROM session_completions"),
        )
    ...
    private static func databaseValue(_ value: TrainingExportJSONValue) -> DatabaseValue {
        switch value {
        case .null: .null
        case let .boolean(value): value.databaseValue
        case let .integer(value): value.databaseValue
        case let .number(value): value.databaseValue
        case let .string(value): value.databaseValue
        case let .blob(value): (Data(base64Encoded: value) ?? Data()).databaseValue
        }
    }
    ...
    private static func correctionSnapshot(
        ...
        let status: TrainingSessionStatus = if session.status == .completed || completion != nil {
            .completed
    ...
    private static func sessionCorrectionAuditEntry(from row: Row) throws
        -> SessionCorrectionAuditEntry
    {
        try SessionCorrectionAuditEntry(
            id: row["id"],
            cycleID: row["cycle_id"],
            sessionID: row["session_id"],
            occurredAt: row["occurred_at"],
            note: row["note"],
            before: correctionSnapshot(fromJSON: row["before_json"]),
            after: correctionSnapshot(fromJSON: row["after_json"]),
        )
    }
    ...
    private static func projectedSession(
        ...
        guard
            ...
            let status: TrainingSessionStatus = if session.status == .unperformed {
                .unperformed
    ...
    private static func stableExportRecordID(
        ...
        if let id = fields["id"], case let .string(value) = id {
            return value
        }
        ...
        let parts = identityColumns.compactMap { column -> String? in
            ...
            switch value {
            case let .string(string): return "\(column)=\(string)"
            case let .integer(integer): return "\(column)=\(integer)"
            default: return nil
            }
        }
        if !parts.isEmpty {
    ...
    private static func preferenceKey(for record: TrainingExportRecord) -> String? {
        for field in ["key", "name", "id"] {
            if let value = record.fields[field], case let .string(string) = value, !string.isEmpty {
                return string
            }
        }
        return record.id
    ...
    private static func exportValue(_ value: DatabaseValue) -> TrainingExportJSONValue {
        ...
        case let .string(value):
            .string(value)
        case let .blob(value):
            .blob(base64: value.base64EncodedString())
        }
    }
    ...
    private static func auditEntry(from row: Row) throws -> LiftConfigurationAuditEntry {
        ...
        guard beforeValues.allSatisfy(\.self) || beforeValues.allSatisfy({ !$0 }) else {
        ...
        {
            before = try LiftConfigurationSnapshot(
                identity: identity(
        ...
        return try LiftConfigurationAuditEntry(
            id: row["id"],
            liftID: row["lift_id"],
            action: action,
            occurredAt: row["occurred_at"],
